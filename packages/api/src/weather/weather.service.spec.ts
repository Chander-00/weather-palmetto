...
describe('WeatherService', () => {
  ...
  beforeEach(() => {
    ...
    accuWeather = {
      ...
      priority: 2,
      isAvailable: vi.fn().mockReturnValue(true),
      getWeatherByCity: vi.fn().mockResolvedValue({ ...mockWeatherData, provider: 'accuweather' }),
      getWeatherByCoordinates: vi.fn().mockResolvedValue({ ...mockWeatherData, provider: 'accuweather' }),
    };
    ...
    weatherGov = {
      ...
      priority: 3,
      isAvailable: vi.fn().mockReturnValue(true),
      getWeatherByCity: vi.fn().mockResolvedValue({ ...mockWeatherData, provider: 'weathergov' }),
      getWeatherByCoordinates: vi.fn().mockResolvedValue({ ...mockWeatherData, provider: 'weathergov' }),
    };
  ...
  it('should fallback to second provider when primary fails', async () => {
    (openWeather.getWeatherByCity as ReturnType<typeof vi.fn>).mockRejectedValue(new Error('API down'));
    const result = await service.getWeatherByCity('London');
    expect(result.provider).toBe('accuweather');
  });

  it('should fallback to third provider when first two fail', async () => {
    (openWeather.getWeatherByCity as ReturnType<typeof vi.fn>).mockRejectedValue(new Error('fail'));
    (accuWeather.getWeatherByCity as ReturnType<typeof vi.fn>).mockRejectedValue(new Error('fail'));
    const result = await service.getWeatherByCity('London');
  ...
  it('should throw when all providers fail', async () => {
    (openWeather.getWeatherByCity as ReturnType<typeof vi.fn>).mockRejectedValue(new Error('fail'));
    (accuWeather.getWeatherByCity as ReturnType<typeof vi.fn>).mockRejectedValue(new Error('fail'));
    (weatherGov.getWeatherByCity as ReturnType<typeof vi.fn>).mockRejectedValue(new Error('fail'));
    await expect(service.getWeatherByCity('London')).rejects.toThrow('All weather providers failed');
  });
  ...
  it('should skip unavailable providers', async () => {
    (openWeather.isAvailable as ReturnType<typeof vi.fn>).mockReturnValue(false);
    const result = await service.getWeatherByCity('London');
    expect(result.provider).toBe('accuweather');
    expect(openWeather.getWeatherByCity).not.toHaveBeenCalled();
  });
  ...
  it('should work with coordinates', async () => {
    ...
    expect(result.provider).toBe('openweather');
    expect(openWeather.getWeatherByCoordinates).toHaveBeenCalledWith(51.5, -0.12);
  });
});
