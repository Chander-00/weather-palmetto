...
import { WeatherProvider, NormalizedWeather } from './interfaces/weather-provider.interface';
...
export class WeatherService {
  ...
  constructor(
    ...
    this.providers = [openWeather, accuWeather, weatherGov].sort(
      (a, b) => a.priority - b.priority,
    );
  }

  async getWeatherByCity(city: string, units: string = 'imperial'): Promise<NormalizedWeather> {
    const cacheKey = `weather:city:${city.toLowerCase()}:${units}`;
    const cached = await this.cache.get<NormalizedWeather>(cacheKey);
    if (cached) return cached;

    const result = await this.fetchWithFallback((provider) => provider.getWeatherByCity(city));
