...
export class WeatherGovProvider implements WeatherProvider {
  ...
  private async fetchWeather(
    ...
    const pointsResponse = await axios.get(`${BASE_URL}/points/${lat.toFixed(4)},${lon.toFixed(4)}`, { headers });
    const { forecastOffice, gridX, gridY, relativeLocation } = pointsResponse.data.properties;
    ...
    const [forecastResponse, stationsResponse] = await Promise.all([
      axios.get(`${BASE_URL}/gridpoints/${pointsResponse.data.properties.gridId}/${gridX},${gridY}/forecast`, { headers }),
    ...
    if (stationsResponse?.data?.features?.length) {
      ...
      try {
        ...
        this.logger.warn('Failed to fetch station observations, using forecast data');
      }
    }
  ...
  private normalize(
    ...
    const dailyMap = new Map<string, { highs: number[]; lows: number[]; period: any }>();
    ...
    const forecastDays: ForecastDay[] = Array.from(dailyMap.entries())
      ...
      .map(([date, { highs, lows, period }]) => ({
        ...
        temperatureLow: lows.length ? Math.min(...lows) : period.temperature - 10,
    ...
    return {
      ...
      current: {
        ...
        feelsLike: observation ? this.fahrenheitValue(observation.windChill || observation.temperature) : currentTemp,
        humidity: observation?.relativeHumidity?.value ?? 0,
