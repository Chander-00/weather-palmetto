...
export class AccuWeatherProvider implements WeatherProvider {
  ...
  async getWeatherByCity(city: string): Promise<NormalizedWeather> {
    const searchResponse = await axios.get(`${BASE_URL}/locations/v1/cities/search`, {
  ...
  async getWeatherByCoordinates(lat: number, lon: number): Promise<NormalizedWeather> {
    const searchResponse = await axios.get(`${BASE_URL}/locations/v1/cities/geoposition/search`, {
  ...
  private async fetchWeatherByLocationKey(location: any): Promise<NormalizedWeather> {
  ...
  private normalize(location: any, current: any, forecast: any): NormalizedWeather {
    const forecastDays: ForecastDay[] = forecast.DailyForecasts.slice(0, 5).map((day: any) => ({
    ...
    return {
      ...
      current: {
        ...
        feelsLike: Math.round(current.RealFeelTemperature?.Imperial?.Value ?? current.Temperature.Imperial.Value),
        ...
        visibility: current.Visibility?.Imperial?.Value ? current.Visibility.Imperial.Value * 1609.34 : 10000,
