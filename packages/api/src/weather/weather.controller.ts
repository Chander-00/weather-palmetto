...
export class WeatherController {
  ...
  async getWeather(@Query() query: WeatherQueryDto): Promise<WeatherResponseDto> {
    if (!query.city && (query.lat === undefined || query.lon === undefined)) {
      throw new BadRequestException('Provide either a city name or lat/lon coordinates');
    }

    try {
      if (query.city) {
        return await this.weatherService.getWeatherByCity(query.city, query.units);
      }
      return await this.weatherService.getWeatherByCoordinates(query.lat!, query.lon!, query.units);
    } catch (error) {
