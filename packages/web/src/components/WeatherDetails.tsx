...
export function WeatherDetails({ current, units }: WeatherDetailsProps) {
  ...
  const details = [
    ...
    {
      label: 'Wind',
      value: `${current.windSpeed} ${speedUnit} ${getWindDirection(current.windDirection)}`,
      icon: '\u{1F4A8}'
    },
    ...
    {
      label: 'UV Index',
      value: current.uvIndex !== null ? String(current.uvIndex) : 'N/A',
      icon: '\u{2600}\u{FE0F}'
    },
    {
      label: 'Feels Like',
      value: `${current.feelsLike}\u{00B0}`,
      icon: '\u{1F321}\u{FE0F}'
    }
  ]
